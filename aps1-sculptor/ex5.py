...
def realiza_diferencas(bgr): 
    ...
    # Forneça como saída uma uma cópia da imagem em *tons de cinza* em que cada pixel  
    # recebe o módulo da subtração entre o pixel que vem depois e o que vem antes dele (na horizontal).
    gray_copy = np.zeros_like(gray)

    for i in range(gray.shape[0]):
        for j in range(gray.shape[1]):
            if j == 0:
                gray_copy[i, j] = (abs(gray[i, j+1] - gray[i, j]))
            elif j == gray.shape[1]-1:
                gray_copy[i, j] = abs(gray[i, j] - gray[i, j-1])
            else:
                gray_copy[i, j] = abs(gray[i, j+1] - gray[i, j-1])
